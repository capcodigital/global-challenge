...
const buildRequest = (opt, callback) => {
...
const userRequest = (opt, callback) => {
  const req = buildRequest(opt, callback);
  req.end();
};

const getAccessToken = (opt, callback) => {
...
const buildPhotoRequest = (opt, callback) => {
  const req = https.request(opt, (res) => {
    const buffers = [];
    let length = 0;
...
export const getUserPhoto = (userId, callback) => {
  const getUserPhotoOptions = options;
